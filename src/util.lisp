    ...
    (with-vector-parsing (string)
      (labels ((skip-to (char)
               ...
               (read-object ()
                 (advance)
                 (skip-spaces)
                 (loop until (and (char= (current) #\}) (advance!))
                       nconc (list (make-keyword (read-string))
               ...
               (read-string ()
                 (with-output-to-string (stream)
                   (loop for chr = (advance)
                         until (and (char= chr #\") (advance!))
                         if (char= chr #\\)
                           do (write-char
                               (case (the standard-char (advance))
                                 (#\b #\Backspace)
                                 (#\f #\Newline)
                                 (#\n #\Newline)
                                 (#\r #\Return)
                   ...
                   (bind (num-str (skip-while integer-char-p))
                     (let ((num (parse-integer num-str)))
                       (if (and (not rest-p)
                           ...
                           (progn (advance)
                                  (+ num
                                     (multiple-value-bind (rest len) (read-number)
                                       (/ rest (expt 10 len)))))
                           (values num (- (pos) start))))))))
        (skip-to #\{)
        (setq result (dispatch))))
    result))
